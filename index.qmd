...
## Times series 2018-2019 outbreak
...
cases_hcm_1819$onset_month <- format(cases_hcm_1819$admission, format = "%b-%Y")

cases_hcm_1819$onset_month <- factor(cases_hcm_1819$onset_month, 
                               levels = paste(rep(month.abb[1:12], times = 2), 
                                              rep(c(2018, 2019), each = 12), 
                                              sep = "-"))

df_plot2 <- cases_hcm_1819 |> 
  mutate(district = district %>% 
           str_replace_all(
             c("Quận 2" = "Thủ Đức",
               "Quận 9" = "Thủ Đức")) %>% 
           ...
           stri_trans_general("latin-ascii") %>% 
           tolower()) %>% 
  group_by(onset_month,district) |> 
  count() |> 
  as.data.frame() |> 
  complete(onset_month, district, fill = list(n = 0)) |>
  arrange(onset_month) |> 
  filter(!onset_month %in% paste(month.abb[1:4], 2018, sep = "-")) |>
  group_by(district) |> 
  mutate(
    cumn = cumsum(n)
  )
...
df_plot2 %>% 
  left_join(.,hcm_pop_19, by = join_by(district)) %>% 
  mutate(cuminc = cumn/pop*100000,
         cuminc = case_when(
           cuminc == 0 ~ NA,
           cuminc != 0 ~ cuminc
         )) %>% 
  ...
  group_modify(~.x %>%
                 left_join(qhtp, ., by = join_by(varname_2 == district))) %>% 
  ggplot() +
  geom_sf(aes(fill = cuminc,geometry = geom)) +
  scale_fill_gsea(
  ...
  facet_wrap(~ onset_month, nrow = 4)+
  labs(fill = "Cumulative incidence rate \n(per 100,000 population)") +
  theme_bw() +
  theme(
    legend.position = "top",
